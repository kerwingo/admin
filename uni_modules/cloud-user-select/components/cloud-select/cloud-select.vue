<template>
	<view class="uni-data-checklist">
		<uni-forms ref="form">
			<view class="uni-easyinput" @click="activeChange(true)">
				<view class="uni-easyinput__content is-input-border" style="padding-left: 20rpx;">
					<text v-if="result">{{result}}</text>
					<text v-else>{{placeholder}}</text>
				</view>
			</view>
		</uni-forms>
		<view v-if="active" class="uni-datetime-picker-mask" @click="activeChange(false)"></view>
		<block v-if="active">
			...
			<template v-else>
				<scroll-view scroll-y="true" class="more-list" @scrolltolower="loadMore">
					<view>
...
<script>
	...
	export default {
		...
		created() {
			...
			if(this.defaultResult && !this.result){
				this.result = this.defaultResult;
				console.log("this.defaultResult",this.defaultResult)
			}
		},
		methods: {
			init(range) {},
			...
			chagne(e) {
				...
				if (detail.data && detail.data.is_other) {
			...
			setRange(list) {
				...
				this.result = selectList.map(s=>s.text).join("、")
			...
			changeOtherText(val) {
				if (this.formItem && this.formItem.name) {
					if (val) {
						this.form.formData[this.formItem.name + "_other"] = val;
					} else {
						delete this.form.formData[this.formItem.name + "_other"];
					}
					console.log(this.formItem.name + "_other", val)
