	...
	<view>
		...
		<view class="uni-container">
			<unicloud-db ref="udb" :collection="collectionName" :options="options" :where="where" page-data="replace"
				:orderby="orderby"
				field="title,category_id,article_status,comment_status,avatar,publish_date,last_modify_date"
				...
				<uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						...
						<uni-td align="center">{{item.category_id?item.category_id.map(e=>e.name).join("、"):""}}
						</uni-td>
						<uni-td align="center"> {{item.article_status == true ? '✅' : '❌'}} </uni-td>
						<uni-td align="center"> {{item.comment_status == true ? '✅' : '❌'}} </uni-td>
						<uni-td align="center">
...
<script>
	...
	// 表查询配置
	const dbCollectionName = 'opendb-news-articles,opendb-news-categories'
